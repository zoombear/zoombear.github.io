...
function selectSamples(theForm,theList,theStatusForm)
{
	...
	//Checks to see if search type is by sample collection number. If so, the search is conducted for the specific collection number.
	else if(theSearch=="sample")
	{
	...
	//Selects the specified samples from the Fusion Table (this search finds records containing the search clause regardless of case in the Fusion Table)
	//Search may find records with names including the search phrase since it uses the "contains" select query.
	else
	{
		...
		var newLayer = new google.maps.FusionTablesLayer({
			query:
			{
				...
				//This limits the data from the Fusion Table to be displayed
				where: "'" + searchField + "' CONTAINS IGNORING CASE '" + theSample + "'"
			},
